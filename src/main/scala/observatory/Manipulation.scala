...
/**
  ...
  * If you tried to visualize the temperatures of different years in the previous milestone,
  * you might have noticed that it is actually quite hard to really measure how
  * the temperatures have evolved since 1975.
  *
  * That’s why we propose to visualize the deviations of the temperatures over the years,
  * rather than just the temperatures themselves.
  * The goal of this milestone is to compute such deviations.
  *
  * Computing deviations means comparing a value to a previous value which serves as a reference,
  * or a “normal” temperature.
  * You will first compute the average temperatures all over the world between 1975 and 1990.
  * This will constitute your reference temperatures, which we refer to as “normals”.
  ...
  * The above figure illustrates the grid points (in green) and the actual data points (in red).
  * You will have to guess the temperature at the green locations based on the known temperatures
  * at the red locations.
  *
  * Once you will have such a grid for each year, you will easily be able to compute average
  * (coordinate wise) over years and deviations.
  *
  *
  * ***************
  * Grid generation
  * ***************
  * To describe a grid point's location, we'll use integer latitude and longitude values.
  ...
  * You are free to internally represent the grid as you want (e.g. using a class Grid),
  * but to inter-operate with the grading system you will have to convert it to a function of
  * type GridLocation => Temperature, which returns the temperature at the given grid location.
  *
  ...
  *
  * When we generate the map, we'll have to get the temperature of every point on the grid at least once.
  * With this information in mind, think about which approach is more suitable and chose the best one for you.
  *
  *
  * *********************************
  ...
  * (each “temperature data” for one year being a sequence of pairs of average yearly temperature and location),
...
object Manipulation extends ManipulationInterface {
  ...
  def average(temperatures: Iterable[Iterable[(Location, Temperature)]]): GridLocation => Temperature = {
    gridLoc =>
      val temps: Iterable[Temperature] = temperatures.map(makeGrid).map(fun => fun(gridLoc))
