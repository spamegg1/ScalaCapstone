...
/**
  ...
  * But now the situation has changed: instead of working with a set of scattered points,
  * you have a regular grid of points:
  *
  * In this figure, the square in the middle materializes a pixel that you want to compute.
  * You can leverage the grid to use a faster interpolation algorithm:
  * you can now use bilinear interpolation rather than inverse distance weighting.
  * More precisely, you will implement a simplified form of bilinear interpolation:
  *
  * In this form of bilinear interpolation, the location of the point to estimate is given by coordinates x and y,
  ...
  * As such, the coordinates of a pixel inside of a grid cell can be described by the following case class,
  * defined in models.scala:
  *       case class CellPoint(x: Double, y: Double)
  *
  * You will also have to decide on a color scale to use, to represent the temperature deviations.
  ...
  * to estimate the temperature at, and the 4 known temperatures as shown in the above figure,
  ...
  * where each pixel has a color computed according to the given color scale applied to the grid values.
  ...
  * **************************
  * Once you have implemented the above methods, you are ready to generate the tiles
  * showing the deviations for all the years between 1990 and 2015,
  * so that the final application (in the last milestone) will nicely display them:
  *
  ...
  *
  * Note that this process is going to be very CPU consuming, or might even crash
  * if your implementation tries to load too much data into memory.
  ...
  *     compute 128×128 images (that’s going to be 4 times fewer pixels to compute)
...
object Visualization2 extends Visualization2Interface {
  ...
  /**
    ...
    * NOTE: COPIED FROM Visualization.visualize(), modified to use gridLocations and bilinearInterpolation
    */
  def visualizeGrid(
    ...
    val predictedTemps: ParSeq[Temperature] = locations.map{case Location(lat, lon) =>
