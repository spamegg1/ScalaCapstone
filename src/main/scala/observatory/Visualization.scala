...
/**
  ...
  * Your work consists in building an image of 360×180 pixels,
  ...
  * As you can see, you will have to spatially interpolate the data in order to guess
  * the temperature corresponding to the location of each pixel
  ...
  * a temperature of 12°C should be represented in yellow (255, 255, 0, 255), and so on.
  * For temperatures between thresholds, say, between 12°C and 32°C,
  * you will have to compute a linear interpolation between the yellow and red colors.
  *
  ...
  * To approximate distance between two locations, we suggest you use great-circle distance formula:
  *     https://en.wikipedia.org/wiki/Great-circle_distance
  *
  * Note that the great-circle distance formula is known to have rounding errors for short distances
  * (a few meters), but that’s not a problem for us because we don’t need such a high degree of precision.
  ...
  *     dist = r * deltaSigma
  *
  * However, running the inverse distance weighting algorithm with small distances
  * will result in huge numbers (since we divide by the distance raised to the power of p),
  * which can be a problem. A solution to this problem is to directly use the known temperature
  * of the close (less than 1 km) location as a prediction.
  *
  *
  * ********************
  ...
  * Visualization
  * *************
  * Once you have completed the above steps you can implement the visualize method to build an image
  * (using the scrimage library) where each pixel shows the temperature corresponding to its location.
  * Note that the (x,y) coordinates of the top-left pixel is (0,0)
  * and then the x axis grows to the right and the y axis grows to the bottom, to (359,179),
  * whereas the latitude and longitude origin, (0,0), is at the center of the image (180,90),
  ...
  *
  * So... pixel's x location goes from 0 to 359... where longitude goes from -180 to 179
  *      this means that longitude = x - 180
  *       pixel's y location goes from 0 to 179... where latitude goes from 90 to -89
  ...
  * Also, note that scrimage defines a Color type, which could be ambiguous with our Color definition.
...
object Visualization extends VisualizationInterface {
  ...
  def predictTemperature(temperatures: Iterable[(Location, Temperature)], location: Location): Temperature = {
    if (temperatures.exists(_._1 == location)) /* location = one of given locations */
      ...
      distances.find(_._1 < 1.0) match {
        case Some((_, temp)) => temp /* small distance, use known temp of close loc */
        case None => /* all distances are > 1 */
          val weights: Iterable[Double] = distances.map(_._1).map(pow(_, -p))
          val numer: Double = weights.zip(temperatures.map(_._2)).map { case (w, t) => w * t }.sum
  ...
  def interpolateColor(points: Iterable[(Temperature, Color)], value: Temperature): Color = {
    val sortedPts: List[(Temperature, Color)] = points.toList.sortBy(_._1)
    if (sortedPts.exists(_._1 == value)) sortedPts.find(_._1 == value).get._2
      ...
      val red: Double = lineSeg(lower._1, lower._2.red, upper._1, upper._2.red, value)
      val green: Double = lineSeg(lower._1, lower._2.green, upper._1, upper._2.green, value)
      val blue: Double = lineSeg(lower._1, lower._2.blue, upper._1, upper._2.blue, value)
  ...
  def visualize(temperatures: Iterable[(Location, Temperature)],
    ...
    val locations: ParSeq[Location] =
      pixels.par.map{case (x, y) => Location(90 - x, y - 180)} /* HAD TO SWAP THESE! */
    ...
    val coloredLocations: ParSeq[Color] =
      predictedTemps.map{case (_, temp) =>
        interpolateColor(colors, temp)}

    val pixelArray: Array[Pixel] =
      coloredLocations.map(
        color => Pixel(color.red, color.green, color.blue, 255))
        .toArray
